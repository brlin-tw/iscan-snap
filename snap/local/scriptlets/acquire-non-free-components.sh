...
if ! test -v CRAFT_PART_SRC; then
    printf -- \
        '%s: Error: The CRAFT_PART_SRC environment variable is not set, this program is intended to be run from within a Snapcraft build environment.\n' \
        "${script_name}" \
        1>&2
    exit 1
fi
...
if ! \
    ...
    --silent \
    --show-error \
    --user-agent 'iscan-snap-non-free-components-acquisition-script/0.0' \
...
done < "${CRAFT_PART_SRC}/non-free/checksums.md5"

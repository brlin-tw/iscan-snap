/*
  Hook fopen() to evade hardcoded paths in applications
...
FILE* fopen_wrapper(const char *path, const char *mode){
    fprintf(stderr, "DEBUG: fopen_wrapper: fopen() function hook called.\n");
    ...
    FILE *open_result = NULL;
    switch(match_result){
        case 0:
            fprintf(stderr, "DEBUG: fopen_wrapper: Hardcoded path detected.\n");
            ...
            fprintf(stderr, "DEBUG: fopen_wrapper: Remaining path determined to be = \"%s\".\n", remaining_path);
            ...
            printf("DEBUG: fopen_wrapper: Hardcoded path patched to \"%s\".\n", patched_path);
            open_result = fopen(patched_path, mode);
            free(patched_path);
            break;

        case FNM_NOMATCH:
            open_result = fopen(path, mode);
            break;

        default:
            fprintf(stderr, "Error: fopen_wrapper: Unknown fnmatch error.\n");
            exit(EXIT_FAILURE);
            break;
    }
    return open_result;
